...
add_action( 'wp_ajax_get_marketplace_shop_fields_html', 'get_marketplace_shop_fields_html' );

function get_marketplace_shop_fields_html() {
	...
	if ( $ajax_nonce ) {
		...
		$product_id  = $post['product_id'] ?? 0;
		$site_id     = $post['site_id'] ?? 0;
		$shop        = $post['shop'] ?? array();
		include_once dirname( __FILE__ ) . '/classes/ced-mbc-render-fields.php';
		$fields_obj = new Ced_MBC_Render_Fields( $product_id );
		$html       = $fields_obj->get_marketplace_shop_fields_html( $marketplace, $shop );
